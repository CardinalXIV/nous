...
def recognize_intent(user_message):
    # Using the classifier to detect if the intent is a question (query) or reminder
    ...
    if classification['label'] == 'LABEL_1':  # Adjust this based on your classifier's output
        # If the classifier thinks it's positive, we treat it as a query
        return "query"
...
def clean_reminder_text(user_message, parsed_date):
    # Replace time-related phrases more carefully
    ...
    phrases_to_remove = [
        ...
        r'\bat\b\s*\d{1,2}\s*(am|pm)',
        r'\b(\d{1,2}[:.]\d{2}\s*(am|pm)?|\d{1,2}\s*(am|pm))',
        r'\b(morning|afternoon|evening|night)\b',
        r'\btomorrow\b',
        r'\btoday\b',
        r'\bon\b\s*\b\d{1,2}/\d{1,2}/\d{2,4}\b'
    ]

    # Process the text and remove the above phrases
